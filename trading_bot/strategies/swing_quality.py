...
class SwingQualityStrategy(BaseStrategy):
    ...
    def min_bars(self) -> int:
        return max(self.indicators.max_lookback, self.config.long_ema + self.config.pullback_window + 5)
    ...
    def evaluate(self, symbol: str, data: pd.DataFrame) -> Signal | None:
        ...
        healthy_volume = relative_volume is not None and relative_volume >= self.config.min_relative_volume

        conditions = [
            Condition("primary_trend", trend_stack, weight=2.5, required=True,
                      detail=f"Bullish EMA stack: {self.config.fast_ema} > {self.config.mid_ema} > {self.config.long_ema}"),
            Condition("entry_trigger", trigger, weight=2.5, required=True,
                      detail=("Constructive pullback reclaimed the 20 EMA" if pullback_reclaim else f"Fresh {self.config.breakout_lookback}-bar breakout")),
            Condition("momentum_room", rsi_room, weight=1.5, required=True,
                      detail=(f"RSI {rsi:.1f} is in the swing entry zone" if rsi is not None else "RSI unavailable")),
            Condition("ema_slopes", fast_rising and mid_rising, weight=1.25,
                      detail="20 EMA and 50 EMA are both rising"),
            Condition("trend_quality", trend_quality, weight=1.25,
                      detail=f"Trend strength {trend.strength}/100"),
            Condition("volume_confirmation", healthy_volume, weight=1.0,
                      detail=(f"Relative volume {relative_volume:.2f}x" if relative_volume is not None else "Relative volume unavailable")),
        ]
