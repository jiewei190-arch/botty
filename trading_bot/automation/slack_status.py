...
def main() -> None:
    ...
    # Imported here rather than at module scope: Slack is one optional way to
    # read status, and `status_message` — which is what everything else calls —
    # needs none of it. A top-level import would make slack-bolt a hard
    # dependency of the whole package for a feature most runs never touch.
    try:
        from slack_bolt import App
        from slack_bolt.adapter.socket_mode import SocketModeHandler
    except ModuleNotFoundError as error:  # pragma: no cover - optional extra
        raise SystemExit(
            "Slack status needs the optional dependency: pip install slack-bolt"
        ) from error
